 ...
 */
function express_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name.
  $form['site_information']['site_name']['#default_value'] = st('My Backdrop Site');

  $form['#submit'][] = 'express_form_install_configure_submit';
}

/**
 * Extra submit handler install_configure_form().
 */
function express_form_install_configure_submit($form, &$form_state) {
